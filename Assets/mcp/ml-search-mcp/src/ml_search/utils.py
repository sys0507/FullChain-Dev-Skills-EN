...
def extract_unique_urls(text: str) -> list[str]:
    """Extract unique URLs in first-appearance order."""
    seen: set[str] = set()
    urls: list[str] = []
...
- **Language**: Preserve the source page's language unless the user explicitly requests a translation.
- **Role**: Fetch the requested URL and convert its meaningful content into faithful, structured Markdown.

## Workflow

1. Validate the URL and follow safe redirects.
2. Detect the page title, table of contents, headings, paragraphs, lists, tables, code, media, and links.
3. Preserve the page hierarchy and semantic meaning.
4. Remove scripts, styles, tracking code, advertisements, and unrelated navigation noise.
5. Return Markdown with a metadata header containing `source`, `title`, and `fetched_at`.

## Rules

- Preserve the original facts, wording, language, and document structure.
- Do not summarize, rewrite, translate, or omit substantive content unless explicitly requested.
- Keep code formatting, link targets, image alt text, and table structure intact.
- Use UTF-8 and valid Markdown.
"""

# Backward-compatible default for callers that import fetch_prompt directly.
fetch_prompt = fetch_prompt_en
