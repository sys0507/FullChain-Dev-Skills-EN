"""ml-search-mcp runtime configuration.

Env-var prefix: ML_*  (e.g. ML_API_KEY, ML_MODEL).

Provider modes (ML_PROVIDER):
  ...
  - custom:               user supplies ML_API_URL & ML_MODEL themselves
"""
...
class Config:
    ...
    _SETUP_HINT = (
        ...
        "  uvx --from ml-search-mcp ml-search-setup\n"
        "Or set ML_API_KEY (required) and ML_PROVIDER "
        "(openrouter|xai|custom) manually."
    )
    ...
    def config_file(self) -> Path:
        if self._config_file is None:
            config_dir = Path.home() / ".config" / "ml-search"
            try:
                config_dir.mkdir(parents=True, exist_ok=True)
            except OSError:
                config_dir = Path.cwd() / ".ml-search"
                config_dir.mkdir(parents=True, exist_ok=True)
    ...
    def _save_config_file(self, config_data: dict) -> None:
        try:
            ...
            raise ValueError(f"Unable to save the configuration file: {str(e)}")

    # ── provider selection ─────────────────────────────────────────────

    @property
    def provider(self) -> str:
        v = (_env("ML_PROVIDER") or "").strip().lower()
    ...
    def grok_api_url(self) -> str:
        url = _env("ML_API_URL", "GROK_API_URL")
        if not url:
            url = _PROVIDER_DEFAULTS[self.provider]["api_url"]
        if not url:
            raise ValueError(f"API URL is not configured.\n{self._SETUP_HINT}")
        return url

    @property
    def grok_api_key(self) -> str:
        key = _env("ML_API_KEY", "GROK_API_KEY", "OPENROUTER_API_KEY")
        if not key:
            raise ValueError(f"API key is not configured.\n{self._SETUP_HINT}")
        return key
    ...
    def openrouter_referer(self) -> Optional[str]:
        # Private deployments do not advertise a repository URL by default.
        return _env("ML_OPENROUTER_REFERER", default="") or None

    @property
    def openrouter_title(self) -> Optional[str]:
        return _env("ML_OPENROUTER_TITLE", default="ml-search-mcp")
    ...
    def output_language(self) -> str:
        """Return the requested response language: auto, en, or zh."""
        value = (_env("ML_OUTPUT_LANGUAGE", default="auto") or "auto").strip().lower()
        aliases = {
            "automatic": "auto",
            "english": "en",
            "en-us": "en",
    ...
    def debug_enabled(self) -> bool:
        return _bool("ML_DEBUG", "GROK_DEBUG")

    @property
    def retry_max_attempts(self) -> int:
        return int(_env("ML_RETRY_MAX_ATTEMPTS", "GROK_RETRY_MAX_ATTEMPTS", default="3"))

    @property
    def retry_multiplier(self) -> float:
        return float(_env("ML_RETRY_MULTIPLIER", "GROK_RETRY_MULTIPLIER", default="1"))

    @property
    def retry_max_wait(self) -> int:
        return int(_env("ML_RETRY_MAX_WAIT", "GROK_RETRY_MAX_WAIT", default="10"))

    # ── Tavily / Firecrawl ─────────────────────────────────────────────

    @property
    def tavily_enabled(self) -> bool:
        return _bool("ML_TAVILY_ENABLED", "TAVILY_ENABLED", default=True)
    ...
    def log_level(self) -> str:
        return (_env("ML_LOG_LEVEL", "GROK_LOG_LEVEL", default="INFO") or "INFO").upper()

    @property
    def log_dir(self) -> Path:
        log_dir_str = _env("ML_LOG_DIR", "GROK_LOG_DIR", default="logs") or "logs"
        log_dir = Path(log_dir_str)
        if log_dir.is_absolute():
            return log_dir

        home_log_dir = Path.home() / ".config" / "ml-search" / log_dir_str
        try:
        ...
        tmp_log_dir = Path("/tmp") / "ml-search" / log_dir_str
    ...
    def get_config_info(self) -> dict:
        ...
        return {
            "ML_PROVIDER": self.provider,
            "ML_API_URL": api_url,
            "ML_API_KEY": api_key_masked,
            "ML_MODEL": self.grok_model,
            "ML_OUTPUT_LANGUAGE": self.output_language,
            "ML_DEBUG": self.debug_enabled,
            "ML_LOG_LEVEL": self.log_level,
            "ML_LOG_DIR": str(self.log_dir),
