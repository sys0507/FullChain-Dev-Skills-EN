...
# 尝试使用绝对导入（支持 mcp run）
try:
    from ml_search.providers.grok import GrokSearchProvider
    from ml_search.logger import log_info
    from ml_search.config import config
    from ml_search.sources import SourcesCache, merge_sources, new_session_id, split_answer_and_sources
    from ml_search.planning import engine as planning_engine, result_cache, _split_csv
except ImportError:
...
@mcp.tool(
    ...
    output_schema=None,
    description="""
    Performs a deep web search via Grok and returns its answer with cached source list.
...
async def web_search(
    ...
    try:
        ...
        return {"session_id": session_id, "content": f"Configuration error: {str(e)}", "sources_count": 0}
    ...
    if model:
        ...
        if available and model not in available:
            await _SOURCES_CACHE.set(session_id, [])
            return {"session_id": session_id, "content": f"Invalid model: {model}", "sources_count": 0}
        effective_model = model

    grok_provider = GrokSearchProvider(api_url, api_key, effective_model, config.provider_extra_headers())
        ...
        try:
            async with httpx.AsyncClient(timeout=90.0) as client:
                ...
                if markdown and markdown.strip():
                    return markdown
                await log_info(ctx, f"Firecrawl returned empty Markdown; retry {attempt + 1}/{max_retries}", config.debug_enabled)
...
async def web_fetch(
    ...
    if not config.tavily_api_key and not config.firecrawl_api_key:
        return "Configuration error: neither TAVILY_API_KEY nor FIRECRAWL_API_KEY is configured."
    return "Extraction failed: no configured extraction service could retrieve the page."
...
async def _call_tavily_map(url: str, instructions: str = None, max_depth: int = 1,
    ...
    api_key = config.tavily_api_key
    if not api_key:
        return "Configuration error: set the TAVILY_API_KEY environment variable."
    ...
    try:
        ...
        return f"Site mapping timed out after {timeout} seconds."
    except httpx.HTTPStatusError as e:
        return f"HTTP error: {e.response.status_code} - {e.response.text[:200]}"
    except Exception as e:
        return f"Site mapping error: {str(e)}"
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if response.status_code == 200:
                ...
                test_result["message"] = f"Retrieved model list (HTTP {response.status_code})"
                ...
                try:
                    ...
                    if "data" in models_data and isinstance(models_data["data"], list):
                        ...
                        test_result["message"] += f"; {model_count} models available"
                ...
                test_result["status"] = "⚠️ Connection error"
                test_result["message"] = f"HTTP {response.status_code}: {response.text[:100]}"
                test_result["response_time_ms"] = round(response_time, 2)

    except httpx.TimeoutException:
        test_result["status"] = "❌ Connection timed out"
        test_result["message"] = "The request timed out after 10 seconds. Check the network and API URL."
...
@mcp.tool(
    ...
    **Key Features:**
        ...
        - **Persistent Storage:** Model preference saved to ~/.config/ml-search/config.json.
...
async def switch_model(
    ...
    try:
        ...
        result = {
            "status": "❌ Failed",
            "message": f"Unable to switch model: {str(e)}"
        }
        return json.dumps(result, ensure_ascii=False, indent=2)
    except Exception as e:
        result = {
...
async def toggle_builtin_tools(
    ...
    if action in ["on", "enable"]:
        ...
        msg = "Claude Code built-in web tools are disabled."
        blocked = True
        ...
        with open(settings_path, 'w', encoding='utf-8') as f:
            json.dump(settings, f, ensure_ascii=False, indent=2)
        msg = "Claude Code built-in web tools are enabled."
        blocked = False
    else:
        msg = f"Claude Code built-in web tools are currently {'disabled' if blocked else 'enabled'}."
...
tool must be one of: web_search | web_fetch | web_map.""",
)
async def plan_tool_mapping(
    ...
    for raw in parsed:
        ...
        if raw["tool"] not in {"web_search", "web_fetch", "web_map"}:
            return json.dumps({"error": f"tool must be web_search | web_fetch | web_map, got: {raw['tool']!r}"})
        item: dict = {
