...
def get_local_time_info() -> str:
    """Return local time context for time-sensitive searches."""
    try:
        # 尝试获取系统本地时区
...
def _needs_time_context(query: str) -> bool:
    """Detect Chinese and English time-sensitive queries."""
    # 中文时间相关关键词
    cn_keywords = [
        "当前", "现在", "今天", "明天", "昨天",
        "本周", "上周", "下周", "这周",
...
def _language_instruction() -> str:
    if config.output_language == "en":
        return "Respond in English, even when the query or sources use another language."
    if config.output_language == "zh":
        return "Use Simplified Chinese for the response while preserving citations and proper nouns."
    return "Respond in the same language as the user's query; support both English and Chinese."
...
class GrokSearchProvider(BaseSearchProvider):
    def __init__(self, api_url: str, api_key: str, model: str = "grok-4-fast", extra_headers: Optional[dict] = None):
    ...
    async def fetch(self, url: str, ctx=None) -> str:
        headers = self._base_headers()
        use_chinese = config.output_language == "zh"
        fetch_prompt = fetch_prompt_zh if use_chinese else fetch_prompt_en
        fetch_request = (
            "获取该网页内容并返回结构化 Markdown。"
            if use_chinese
            else "Fetch this page and return its content as structured Markdown."
        )
        payload = {
            "model": self.model,
            "messages": [
