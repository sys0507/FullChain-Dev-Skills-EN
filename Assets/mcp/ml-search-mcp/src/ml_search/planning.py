...
SESSION_DIR = Path(
    os.getenv("ML_STATE_DIR", str(Path.home() / ".config" / "ml-search"))
) / "sessions"
...
def _ensure_dir() -> Path:
    try:
        ...
        # The working directory is a portable fallback on Windows and in
        # restricted runtimes where the user profile is read-only.
        alt = Path.cwd() / ".ml-search" / "sessions"
